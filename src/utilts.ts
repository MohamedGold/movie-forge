...
export const formatResult = (obj: any, mediaType?: MediaType): Film => {
  return {
    ...
    releaseDate: obj.release_date || obj.first_air_date, // تاريخ الإصدار
    rating: obj.vote_average, // تقييم الفيلم أو المسلسل
    views: obj.popularity, // عدد المشاهدات إذا كانت متاحة
    seasons:
      obj.seasons?.map(
        (season: any) =>
          ({
...
export const mergeFilms = (movies: Film[], tvs: Film[], limit = 11) => {
  const arrs: Film[] = []
...
export const youtubeThubnail = (key: string) => {
